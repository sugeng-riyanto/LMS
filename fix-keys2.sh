...
cat > src/lib/supabase/supabase-config.ts << 'EOF'
...
grep "QBpmyNn" src/lib/supabase/client.ts | head -c 50
echo ""
echo "Files written OK. Building..."
sudo rm -rf .next
SKIP_TYPECHECK=1 npm run build && sudo pm2 restart lmsshb --update-env
