      ...
      {createOpen && (
        <div className="space-y-4 py-4 border-y border-border">
          ...
          {TPA_CATEGORIES.map(cat => {
            ...
            const raw = cat.items.reduce((sum, item) => sum + (catScores[item.id] ?? Math.ceil(parseScale(scaleLabel).max / 2)), 0)
            const max = cat.items.length * parseScale(scaleLabel).max
            return (
                  ...
                  {cat.items.map(item => (
                    <div key={item.id} className="flex flex-col sm:flex-row sm:items-center gap-2 py-2 border-b border-border/50 last:border-0">
                      ...
                      <div className="flex items-center gap-3 shrink-0">
                        <input type="range" min={parseScale(scaleLabel).min} max={parseScale(scaleLabel).max} step={1} value={catScores[item.id] ?? Math.ceil(parseScale(scaleLabel).max / 2)}
                        ...
                        <span className={`w-8 h-8 sm:w-10 sm:h-10 flex items-center justify-center rounded-lg text-base sm:text-lg font-bold shadow-sm ${(catScores[item.id] ?? Math.ceil(parseScale(scaleLabel).max / 2)) >= Math.ceil(parseScale(scaleLabel).max * 0.75) ? 'bg-green-100 text-green-700 border border-green-300' : (catScores[item.id] ?? Math.ceil(parseScale(scaleLabel).max / 2)) >= Math.ceil(parseScale(scaleLabel).max * 0.5) ? 'bg-amber-100 text-amber-700 border border-amber-300' : 'bg-red-100 text-red-700 border border-red-300'}`}>
                          {catScores[item.id] ?? Math.ceil(parseScale(scaleLabel).max / 2)}
                        </span>
